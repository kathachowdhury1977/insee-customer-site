...
function ShippingDetailsForm(props) {
 ...
 console.log(shippingtype, 'shippingtype---')
  ...
  var selectShipTo = localStorage.getItem('SHIPTOCODE')
  ...
  useEffect(() => {
    dispatch(orderActions.getShipToDetails(selectShipTo))
  }, [])
                      ...
                      (ship_cond, ind) => {
                        if (ship_cond.value != 'Description') {
                          if (subCategoryName === "BIG BAG") {
                            if (ship_cond.key === 'P2') {
                              return (
                                <option value={ship_cond.key} disabled>
                                  {ship_cond.value}
                                </option>
                              )
                            }
                ...
                >
                  {
                    ...
                    ordertype.ordertype
                      ? ordertype.ordertype.map((order, ind) => {
                        ...
                        return <option selected value={order.key}>{order.value}</option>
                      })
                      : null
                     : 
                      <>
                    ...
                    { ordertype.ordertype
                    ? ordertype.ordertype.map((order, ind) => {
                      return <option value={order.key}>{order.value}</option>
                    })
                    : null}
                    </>
